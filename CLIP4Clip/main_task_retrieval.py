...
def get_args(description='CLIP4Clip on Retrieval Task'):
    ...
    parser.add_argument("--model_base", default=None, type=str, required=False, help="Model base.")
    parser.add_argument("--version", default="qwen_1_5", type=str, required=False, help="Version.")
...
def _run_on_single_gpu(model,batch_sequence_output_list, batch_visual_output_list):
    ...
    for idx1, b1 in enumerate(batch_sequence_output_list):
        ...
        for idx2, b2 in enumerate(batch_visual_output_list):
            ...
            b1b2_logits, *_tmp = get_pred(sequence_output, visual_output, normalization=True)
...
def eval_epoch(args, model, test_dataloader, device, n_gpu):
    ...
    # overwrite the tokenizer_model_max_length of the model
    ...
    with torch.no_grad():
        ...
        for bid, batch in enumerate(test_dataloader):
            # batch result to device
            qry_inputs = batch["qry_inputs"]
            pos_inputs = batch["pos_inputs"]
            # batch = tuple(t.to(device) for t in batch)
            # input_ids, input_mask, segment_ids, video, video_mask = batch
            for key in qry_inputs:
                if isinstance(qry_inputs[key], torch.Tensor):
                    qry_inputs[key] = qry_inputs[key].to(model.device)
                elif isinstance(qry_inputs[key], list):
                    if isinstance(qry_inputs[key][0], torch.Tensor):
                        qry_inputs[key] = [x.to(model.device) for x in qry_inputs[key]]
            
            for key in pos_inputs:
                if isinstance(pos_inputs[key], torch.Tensor):
                    pos_inputs[key] = pos_inputs[key].to(model.device)
                elif isinstance(pos_inputs[key], list):
                    if isinstance(pos_inputs[key][0], torch.Tensor):
                        pos_inputs[key] = [x.to(model.device) for x in pos_inputs[key]]

            if multi_sentence_:
                # multi-sentences retrieval means: one clip has two or more descriptions.
                b, *_t = pos_inputs["input_ids"].size()
                with torch.autocast(enabled=True, dtype=torch.bfloat16, device_type="cuda"):
                    sequence_output = model.encode_multimodal_embeddings(**qry_inputs)
                ...
                if len(filter_inds) > 0:
                    ...
                    for key in pos_inputs:
                        if isinstance(pos_inputs[key], torch.Tensor):
                            # print("pos_inputs[key].shape: ", pos_inputs[key].shape)
                            pos_inputs[key] = pos_inputs[key][filter_inds, ...]
                        else:
                            # print("len(pos_inputs[key]): ", (pos_inputs[key]))
                            pos_inputs[key] = [pos_inputs[key][itm] for itm in filter_inds]
                    with torch.autocast(enabled=True, dtype=torch.bfloat16, device_type="cuda"):
                        visual_output = model.encode_multimodal_embeddings(**pos_inputs)
...
def main():
    ...
    tokenizer, model, image_processor, context_len = load_pretrained_model(model_path, args.model_base, "llava_qwen")
